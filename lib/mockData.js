...
// Mock Batches - Pre-created batches grouped by postal code prefix
export const mockBatches = [
    {
        id: 'BATCH-001',
        name: 'Zone 178 - Batch A',
        postalPrefix: '178',
        orderIds: ['ORD-001', 'ORD-004', 'ORD-007', 'ORD-013', 'ORD-018'],
        status: 'ready',
        createdAt: new Date(Date.now() - 2 * 60 * 60 * 1000),
        assignedPicker: null
    },
    {
        id: 'BATCH-002',
        name: 'Zone 178 - Batch B',
        postalPrefix: '178',
        orderIds: ['ORD-002', 'ORD-006', 'ORD-011', 'ORD-016'],
        status: 'ready',
        createdAt: new Date(Date.now() - 1.5 * 60 * 60 * 1000),
        assignedPicker: null
    },
    {
        ...
        name: 'Zone 178 - Mixed',
        postalPrefix: '178',
        orderIds: ['ORD-003', 'ORD-009', 'ORD-014'],
        status: 'ready',
        createdAt: new Date(Date.now() - 1 * 60 * 60 * 1000),
        assignedPicker: null
    },
    {
        id: 'BATCH-004',
        name: 'Zone 178 - Quick Priority',
        postalPrefix: '178',
        orderIds: ['ORD-005', 'ORD-010', 'ORD-015'],
        status: 'in_progress',
        createdAt: new Date(Date.now() - 3 * 60 * 60 * 1000),
    ...
    {
        id: 'BATCH-005',
        name: 'Zone 178 - Large Orders',
        postalPrefix: '178',
        orderIds: ['ORD-008', 'ORD-012', 'ORD-017'],
        status: 'ready',
        createdAt: new Date(Date.now() - 45 * 60 * 1000),
        assignedPicker: null
    }
...
export function getBatchStats() {
    const readyBatches = mockBatches.filter(b => b.status === 'ready').length;
    const inProgressBatches = mockBatches.filter(b => b.status === 'in_progress').length;
    const totalOrders = mockBatches.reduce((sum, batch) => sum + batch.orderIds.length, 0);
...
// Helper function to get orders in a batch
export function getOrdersInBatch(batchId) {
    const batch = mockBatches.find(b => b.id === batchId);
    if (!batch) return [];

    return mockOrders.filter(order => batch.orderIds.includes(order.id));
}

// Helper function to get batch by ID
export function getBatchById(batchId) {
    return mockBatches.find(b => b.id === batchId);
}